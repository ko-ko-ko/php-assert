 ...
 * Class Yii
 */
class Yii1 implements RequestInterface
{
    /** @type CHttpRequest */
    protected $request;
    ...
    public function get($name, $default = null)
    {
        ...
        if ($this->throwException) {
            return Cast::assert($this->request->getParam($name, $default), $name, $this->exceptionClass);
        }

        return Cast::validate($this->request->getParam($name, $default), $name, $this->skipOnErrors);
    }
    ...
    public function toFloat($name, $default = (float)0)
    {
        ...
        return Cast::validate($this->request->getParam($name, $default), $name, $this->skipOnErrors)->toFloat($default);
    }
    ...
    public function toInt($name, $default = 0)
    {
        ...
        return Cast::validate($this->request->getParam($name, $default), $name, $this->skipOnErrors)->toInt($default);
    }
    ...
    public function toBool($name, $default = false)
    {
        ...
        return Cast::validate($this->request->getParam($name, $default), $name, $this->skipOnErrors)->toBool($default);
    }
}